package com.jsframe.laundry.service.board;

import com.jsframe.laundry.dao.BoardDao;
import com.jsframe.laundry.dao.UserDao;
import com.jsframe.laundry.dto.*;
import com.jsframe.laundry.util.PagingUtil;
...
public class BoardService {
    ...
    private int listCnt = 10; //페이지 당 출력할 게시글 갯수
...
//            mDao.memberPtUpdate(member);
    ...
    private void fileUpload(List<MultipartFile> files, HttpSession session, BoardDto board) throws Exception {
        ...
        for (MultipartFile mf : files) {
            ...
            BoardfileDto bf = new BoardfileDto();
            bf.setBf_number(board.getB_num());
    ...
    public ModelAndView getContent(Integer bnum) {
        ...
        //파일 목록 가져오기
        List<BoardfileDto> fList = bDao.fileSelect(bnum);
        //댓글 목록 가져오기
        ...
        //보여질 페이지(jsp) 이름 지정
        mv.setViewName("board/boardContents");
    ...
    @Transactional
    public Map<String, List<BoardfileDto>> fileDelete(String sysname, int bnum, HttpSession session) {
        Map<String, List<BoardfileDto>> fMap = null;
        ...
        try {
            ...
            if(file.exists()) {//파일이 있을 경우
                if(file.delete()) {//파일 삭제에 성공한 경우
                    //파일 목록 다시 가져오기
                    List<BoardfileDto> fList = bDao.fileSelect(bnum);
                    fMap = new HashMap<String, List<BoardfileDto>>();
                    //파일 목록 맵에 삽입
